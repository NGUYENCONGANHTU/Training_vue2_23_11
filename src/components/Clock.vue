...
export default {
  ...
  computed: {
    currentTime() {
      //hiển thị thời gian hiện tại dưới dạng chuỗi định dạng
      return this.formatTime(this.time);
    },
    timerDisplay() {
      // Hiển thị giờ/phút/giây cho bộ đếm thời gian dưới dạng chuỗi định dạng.
      return this.formatTime(
        new Date(
          this.inputHours * 3600 * 1000 +
            this.inputMinutes * 60 * 1000 +
            this.inputSeconds * 1000
        )
      );
    }
  },
  watch: {
    //theo dõi trạng thái của timerRunning
    timerRunning(newVal) {
      if (newVal) {
        //khi newVal thay đổi --> startCountdown thay đổi
        this.startCountdown();
      } else {
        this.stopCountdown();
      }
    ...
    startCountdown() {
      this.countdownInterval = setInterval(() => {
        if (
          ...
          //if hour, minutes, second === 0 are stop!!!!
          this.timerRunning = false;
